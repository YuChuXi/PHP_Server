...
public class EventsListener implements Listener {
	...
	public void onPlayerJoin(@NotNull PlayerJoinEvent e) {
		bot.sendGroupMessage(
			String.format("[%s] %s 加入了游戏", config.getString("server.name"), e.getPlayer().getName()),
	...
	public void onPlayerQuit(@NotNull PlayerQuitEvent e) {
		bot.sendGroupMessage(
			String.format("[%s] %s 退出了游戏", config.getString("server.name"), e.getPlayer().getName()),
	...
	public void onPlayerChat(@NotNull PlayerChatEvent e) {
		bot.sendGroupMessage(
			String.format("[%s] %s: %s", config.getString("server.name"), e.getPlayer().getName(), e.getMessage()),
			config.getLong("bot.group.repost"), "onPlayerChat");
	}
